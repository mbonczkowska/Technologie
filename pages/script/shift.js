$(function () {
    ...
    function ustawCzas() {
        ...
        if (min === 0 && sec === 0) {
            for (var p = 0; p < schronisko.length; p += 1) {
                if (schronisko[p].jedzenie > 0) {
                    ...
                    dana = 10 * schronisko[p].jedzenie + "px";
                ...
                if (schronisko[p].zabawa > 0) {
                    ...
                    dana = 10 * schronisko[p].zabawa + "px";
	 ...
	 socket.on('zaladuj stare paski', function (data) {
           for (var i = 0; i < data.length; i++) {
               ...
               var $wstawKomunikat = $('.' + data[i].idZwierzaka + ' footer .komunikaty .msgJedzenie');
                $nakarmij.width(data[i].szer);
				schronisko[data[i].idZwierzaka].jedzenie = zmien;
               if(data[i].komunikat)
              $wstawKomunikat.html(data[i].komunikat);
            }
   });
   ...
   socket.on('zaladuj stare paski zabawy', function (data) {
    ...
    function ustaw(schronisko) {
		...
		"<td>" + schronisko[i].boks + "</td> </tr>  </table> <footer><div class=\"paski\"></div> <p><a class=\"btn btn-success btn-lg przygarnij\" id=\"" + schronisko[i].boks + "\">Przygarnij</a></p>" +
		"<div class=\"komunikaty\"><div class=\"msgJedzenie\"></div><br/><div class=\"msgZabawa\"></div><a class=\"btn btn-danger btn-xs ukryj\">x</a></div></footer> </article>";
    ...
    function dodajPaski(licznik) {
        ...
        for (var k = 0; k <= licznik; k++) {
            ...
            if (schronisko[k].gatunek === "Kot") {
                $('.' + k + ' footer .paski .przyciski').html("<a class=\"btn btn-primary btn-sm\">Karma</a>" +
                "<a class=\"btn btn-primary btn-sm\">Mleko</a>" +
                "<a class=\"btn btn-primary btn-sm\">Laser</a>" +
                "<a class=\"btn btn-primary btn-sm\">Mysz</a>");

            }

            if (schronisko[k].gatunek === "Królik") {
                $('.' + k + ' footer .paski .przyciski').html("<a class=\"btn btn-primary btn-sm\">Trawa</a>" +
                "<a class=\"btn btn-primary btn-sm\">Marchew</a>" +
                "<a class=\"btn btn-primary btn-sm\">Tuba</a>" +
                "<a class=\"btn btn-primary btn-sm\">Marchewka</a>");
            }
            if (schronisko[k].gatunek != "Pies" && schronisko[k].gatunek != "Kot" && schronisko[k].gatunek != "Królik") {
                $('.' + k + ' footer .paski .przyciski').html("<a class=\"btn btn-primary btn-sm\">Przysmak</a>" +
                "<a class=\"btn btn-primary btn-sm\">Smakołyk</a>" +
                "<a class=\"btn btn-primary btn-sm\">Zabawka</a>" +
                "<a class=\"btn btn-primary btn-sm\">Inne</a>");
            }
        }
    }
        ...
        for (var k = 0; k <= licznik; k += 1) {
            ...
            $('.' + k + ' footer .paski .przyciski a:nth-child(1)').click(function () {
               ...
               // var jedz = $(this).parent().parent().children().children(".jedzenie");
                ...
                uzytkownik = $('#user').html();
				$('.' + zwierz + ' footer .komunikaty').show();
                if(uzytkownik)
				    komunikat = "[" + nazwaJedzenia+ "] Został nakarmiony przez: " + uzytkownik + "</br>";
                else
                    komunikat = "[" + nazwaJedzenia+ "] Został nakarmiony przez: Anonim</br>";
            ...
            $('.' + k + ' footer .paski .przyciski a:nth-child(3)').click(function () {
			   ...
			   nazwaZabawy =$(this).html();
                uzytkownik = $('#user').html();
                if(uzytkownik)
				komunikat = "[" + nazwaZabawy+ "] Pobawił się z: " + uzytkownik + "</br>";
                else
                komunikat = "[" + nazwaZabawy+ "] Pobawił się z: Anonim</br>";
    ...
    $('#dogs, #cats,#rabbits,#others,#all,#main_page').click(function () {
        ...
        if (this.id === 'dogs') {
            ...
            for (i = 0; i < schronisko.length; i++) {
				...
				"<p><a class=\"btn btn-success btn-lg przygarnij\" id=\"" + schronisko[i].boks + "\">Przygarnij</a></p>" +
                "<div class=\"komunikaty\"><div class=\"msgJedzenie\"></div><br/><div class=\"msgZabawa\"></div><a class=\"btn btn-danger btn-xs ukryj\">x</a></div></footer> </article>";
